...
db.exec(`
  CREATE TABLE IF NOT EXISTS users (
    ...
    name TEXT NOT NULL,
    email TEXT UNIQUE,
    phone TEXT NOT NULL, -- Removed UNIQUE to allow same phone for different roles
  ...
  -- Force update password and role for super admin by email and phone to be absolutely sure
...
async function startServer() {
  ...
  app.post("/api/auth/login", (req, res) => {
    ...
    if (role === 'SUPER_ADMIN' || trimmedPhone === 'admin') {
      ...
      if (!anyUser) {
        ...
        return res.status(401).json({ error: "الرقم غير مسجل" });
      }
      ...
      if (anyUser.role !== role) {
        ...
        if (anyUser.role === 'BUSINESS_OWNER') {
          return res.status(401).json({ error: "هذا الرقم مسجل كحساب منشأة، يرجى تسجيل الدخول من خلال تبويب الأعمال" });
        } else if (anyUser.role === 'CUSTOMER') {
          return res.status(401).json({ error: "هذا الرقم مسجل كحساب أفراد، يرجى تسجيل الدخول من خلال تبويب الأفراد" });
        } else {
          return res.status(401).json({ error: `هذا الرقم مسجل كحساب ${anyUser.role}` });
        }
      }
    ...
    try {
      ...
      if (existingUser) {
        if (existingUser.role === 'BUSINESS_OWNER') {
          return res.status(400).json({ error: "هذا الرقم مسجل مسبقاً كحساب منشأة، يرجى تسجيل الدخول بدلاً من التسجيل." });
        } else if (existingUser.role === 'CUSTOMER') {
          return res.status(400).json({ error: "هذا الرقم مسجل مسبقاً كحساب أفراد، يرجى تسجيل الدخول بدلاً من التسجيل." });
        } else {
          return res.status(400).json({ error: `هذا الرقم مسجل مسبقاً كحساب ${existingUser.role}.` });
        }
      }
      ...
      if (role === 'BUSINESS_OWNER') {
        ...
        db.prepare("INSERT INTO businesses (owner_id, name, center_name, tax_number, tax_certificate, commercial_registration, commercial_certificate, address) VALUES (?, ?, ?, ?, ?, ?, ?, ?)").run(
          userId,
          name || '',
          centerName || '',
          taxNumber || '',
  ...
  app.post("/api/business/update", (req, res) => {
    const { 
      ...
      bookingFee, autoConfirmBookings
    } = req.body;
    ...
    try {
      ...
      db.prepare(`
        ...
        SET center_name = ?, tax_number = ?, commercial_registration = ?, address = ?, map_link = ?, images = ?,
            booking_fee = ?, auto_confirm_bookings = ?
        WHERE owner_id = ?
      `).run(
